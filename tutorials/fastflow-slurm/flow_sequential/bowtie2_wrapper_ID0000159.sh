mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000159
cd $NODE_LOCAL/bowtie2_wrapper_ID0000159
if [ -e $ID0000158/SRR1518471_2.fastq ]; then  /bin/ln -s $ID0000158/SRR1518471_2.fastq ./  ; else scp $ID0000158_node:$ID0000158/SRR1518471_2.fastq ./ ; fi
...
if [ -e $ID0000158/SRR1518471_1.fastq ]; then  /bin/ln -s $ID0000158/SRR1518471_1.fastq ./  ; else scp $ID0000158_node:$ID0000158/SRR1518471_1.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518471 
duration=$(( $(date +%s%3N) - start ))
echo ID0000159,bowtie2_wrapper,$duration,$start,$node6 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/dfman/stat.log
date +%s,%H:%M:%S >> ${0}.log
