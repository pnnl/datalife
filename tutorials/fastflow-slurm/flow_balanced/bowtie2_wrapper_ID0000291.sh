mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000291
cd $NODE_LOCAL/bowtie2_wrapper_ID0000291
...
if [ -e $ID0000290/SRR1518445_1.fastq ]; then  /bin/ln -s $ID0000290/SRR1518445_1.fastq ./  ; else scp $ID0000290_node:$ID0000290/SRR1518445_1.fastq ./ ; fi
...
if [ -e $ID0000290/SRR1518445_2.fastq ]; then  /bin/ln -s $ID0000290/SRR1518445_2.fastq ./  ; else scp $ID0000290_node:$ID0000290/SRR1518445_2.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518445 
duration=$(( $(date +%s%3N) - start ))
echo ID0000291,bowtie2_wrapper,$duration,$start,$node12 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
