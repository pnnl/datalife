mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000299
cd $NODE_LOCAL/bowtie2_wrapper_ID0000299
...
if [ -e $ID0000298/SRR1518456_2.fastq ]; then  /bin/ln -s $ID0000298/SRR1518456_2.fastq ./  ; else scp $ID0000298_node:$ID0000298/SRR1518456_2.fastq ./ ; fi
...
if [ -e $ID0000298/SRR1518456_1.fastq ]; then  /bin/ln -s $ID0000298/SRR1518456_1.fastq ./  ; else scp $ID0000298_node:$ID0000298/SRR1518456_1.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518456 
duration=$(( $(date +%s%3N) - start ))
echo ID0000299,bowtie2_wrapper,$duration,$start,$node12 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
