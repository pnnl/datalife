mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000051
cd $NODE_LOCAL/bowtie2_wrapper_ID0000051
...
if [ -e $ID0000050/SRR1518926_2.fastq ]; then  /bin/ln -s $ID0000050/SRR1518926_2.fastq ./  ; else scp $ID0000050_node:$ID0000050/SRR1518926_2.fastq ./ ; fi
if [ -e $ID0000050/SRR1518926_1.fastq ]; then  /bin/ln -s $ID0000050/SRR1518926_1.fastq ./  ; else scp $ID0000050_node:$ID0000050/SRR1518926_1.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518926 
duration=$(( $(date +%s%3N) - start ))
echo ID0000051,bowtie2_wrapper,$duration,$start,$node2 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
