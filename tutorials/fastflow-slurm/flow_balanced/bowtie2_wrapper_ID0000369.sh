mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000369
cd $NODE_LOCAL/bowtie2_wrapper_ID0000369
if [ -e $ID0000368/SRR1518483_1.fastq ]; then  /bin/ln -s $ID0000368/SRR1518483_1.fastq ./  ; else scp $ID0000368_node:$ID0000368/SRR1518483_1.fastq ./ ; fi
...
if [ -e $ID0000368/SRR1518483_2.fastq ]; then  /bin/ln -s $ID0000368/SRR1518483_2.fastq ./  ; else scp $ID0000368_node:$ID0000368/SRR1518483_2.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518483 
duration=$(( $(date +%s%3N) - start ))
echo ID0000369,bowtie2_wrapper,$duration,$start,$node15 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
