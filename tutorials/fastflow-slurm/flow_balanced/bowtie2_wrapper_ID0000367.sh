mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000367
cd $NODE_LOCAL/bowtie2_wrapper_ID0000367
...
if [ -e $ID0000366/SRR1518931_1.fastq ]; then  /bin/ln -s $ID0000366/SRR1518931_1.fastq ./  ; else scp $ID0000366_node:$ID0000366/SRR1518931_1.fastq ./ ; fi
...
if [ -e $ID0000366/SRR1518931_2.fastq ]; then  /bin/ln -s $ID0000366/SRR1518931_2.fastq ./  ; else scp $ID0000366_node:$ID0000366/SRR1518931_2.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518931 
duration=$(( $(date +%s%3N) - start ))
echo ID0000367,bowtie2_wrapper,$duration,$start,$node15 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
