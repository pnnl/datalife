mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000259
cd $NODE_LOCAL/bowtie2_wrapper_ID0000259
...
if [ -e $ID0000258/SRR1518475_1.fastq ]; then  /bin/ln -s $ID0000258/SRR1518475_1.fastq ./  ; else scp $ID0000258_node:$ID0000258/SRR1518475_1.fastq ./ ; fi
...
if [ -e $ID0000258/SRR1518475_2.fastq ]; then  /bin/ln -s $ID0000258/SRR1518475_2.fastq ./  ; else scp $ID0000258_node:$ID0000258/SRR1518475_2.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518475 
duration=$(( $(date +%s%3N) - start ))
echo ID0000259,bowtie2_wrapper,$duration,$start,$node10 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
