mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000381
cd $NODE_LOCAL/bowtie2_wrapper_ID0000381
...
if [ -e $ID0000380/SRR1518960_2.fastq ]; then  /bin/ln -s $ID0000380/SRR1518960_2.fastq ./  ; else scp $ID0000380_node:$ID0000380/SRR1518960_2.fastq ./ ; fi
...
if [ -e $ID0000380/SRR1518960_1.fastq ]; then  /bin/ln -s $ID0000380/SRR1518960_1.fastq ./  ; else scp $ID0000380_node:$ID0000380/SRR1518960_1.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR1518960 
duration=$(( $(date +%s%3N) - start ))
echo ID0000381,bowtie2_wrapper,$duration,$start,$node15 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
