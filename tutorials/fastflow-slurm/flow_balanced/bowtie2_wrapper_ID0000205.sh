mkdir -p $NODE_LOCAL/bowtie2_wrapper_ID0000205
cd $NODE_LOCAL/bowtie2_wrapper_ID0000205
...
if [ -e $ID0000204/SRR3150349_2.fastq ]; then  /bin/ln -s $ID0000204/SRR3150349_2.fastq ./  ; else scp $ID0000204_node:$ID0000204/SRR3150349_2.fastq ./ ; fi
...
if [ -e $ID0000204/SRR3150349_1.fastq ]; then  /bin/ln -s $ID0000204/SRR3150349_1.fastq ./  ; else scp $ID0000204_node:$ID0000204/SRR3150349_1.fastq ./ ; fi
...
date +%s,%H:%M:%S > ${0}.log
start=$(date +%s%3N)
$PFS_PATH/sra-search-workflow/tools/bowtie2_wrapper SRR3150349 
duration=$(( $(date +%s%3N) - start ))
echo ID0000205,bowtie2_wrapper,$duration,$start,$node8 >> $PFS_PATH/sra-search-workflow/192sras_large/16nodes/flowopt_flow_balanced/stat.log
date +%s,%H:%M:%S >> ${0}.log
